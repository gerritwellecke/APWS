...
WiFi functionality is on the horizon, but I recently redid my home server and I
have no MQTT running right now. This will also likely end up in a home assistant
and not NodeRED as I once used it.
*/
#include <Arduino.h>
...
// amount of waterings per wake cycle, i.e. increments of `pump_delay`[s] of
// water flow
static const int water_levels[4]{2, 4, 6, 8}; // TODO set number of "waterings"

// function declarations
void writeIntervalAndSleep(uint wake_interval);
void waterPlants(const int pump_delay, const int water_levels[],
